...
Object 객체는 객체의 가장 기본으로 아무 것도 상속받지 않은 객체이다. 자바스크립트에서 값을 저장하는 가장 기본적인 단위이다. 앞서 얘기했던 것과 같이 자바스크립트에서 객체를 생성하는 생성자는 Object이다.

```javascript
var obj = new Object();
```

따라서 자바스크립트의 모든 객체는 Object 객체를 상속받으며 우리는 Object 객체를 확장함으로써 모든 객체가 접근할 수 있는 API를 생성할 수 있다.

<br>

### 데이터 타입

데이터 타입은 크게 원시 데이터 타입 (기본 데이터 타입) 과 객체 데이터 타입(참조 데이터 타입) 으로 나뉘어진다.
...
이것을 제외한 모든 데이터 타입은 객체 데이터 타입 이라고 한다.
...
위 코드에서 str.length 에서 "." 은 Object access Operator (객체 접근 연산자)라는 명칭을 가지고 있다. 즉, "." 앞에 오는 것이 객체라는 것을 의미한다. 하지만 위 코드에서 str은 "Kong" 이라는 string을 넣은 변수이기 때문에 분명 객체가 아니다. 하지만 오류가 뜨지 않고  정상적으로 동작하였다. 자바스크립트에서는 이처럼 원시 데이터 타입을 마치 객체처럼 사용하고자 할 때, 자바스크립트 내부적으로 자동으로 객체처럼 잠깐 만들어 주는데 이 객체를 **래퍼 객체 (wrapper object)** 라고한다. 래퍼 객체는 함수를 사용하고자 할 때만 잠시동안 유지되고 그 이후에는 사라진다. 원시 데이터 타입을 객체처럼 사용하고자 할 때 다음과 같은 래퍼 객체가 생성된다.
...
변수 a에 1이라는 값이 들어가고, 변수 b에는 a에 있는 값과 같은 값이 들어간다. 따라서 변수 b의 값을 2로 변경해도 a의 값에는 변화가 없다. 이것을 복제라고 한다. 

<br>

#### 참조

변수의 값이 원시데이터 타입인 경우에는 위 코드와 같이 복제가 일어나지만, 변수의 값이 객체인 경우에는 다음과 같이 참조가 일어난다.
...
참조는 복제와 달리 참조한 변수의 값(객체)에 대한 접근 방법을 얻는 것이기 때문에, 한 객체를 참조한 두 개의 변수 중 하나에 변화가 일어나면 다른 참조한 객체에게도 동일하게 변화가 일어난다.
...
하지만 위 코드의 경우에는 변수 b가 a를 참조한 뒤, b에게 새로운 객체를 생성하였기 때문에 참조가 끊기게된다.
<br>
